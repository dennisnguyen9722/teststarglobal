<?php if ( ! defined( 'ABSPATH' ) ) :
    ...
    class TT_Latest_Post_Widget extends WP_Widget {
        public function __construct() {
            parent::__construct(
                'creptaam_latest_post', // Base ID
                esc_html__('Creptaam Latest Posts', 'tt-pl-textdomain'), // Name
                array('description' => esc_html__('Displays latest posts', 'tt-pl-textdomain')) // Args
            );
        }
        ...
        public function form($instance) {
            ...
            <p>
                <label for="<?php echo $this->get_field_id('post_type'); ?>"><?php esc_html_e('Select post type:', 'tt-pl-textdomain'); ?></label>
                <select class="widefat" id="<?php echo $this->get_field_id('post_type'); ?>" name="<?php echo $this->get_field_name('post_type'); ?>" style="width:100%;">
                    <option value="post" <?php selected($instance['post_type'], 'post'); ?>><?php esc_html_e('Post', 'tt-pl-textdomain') ?></option>
                </select>
            </p>

            <p>
                <label for="<?php echo $this->get_field_id('title_length'); ?>"><?php esc_html_e('Post Title Limit (in word): ', 'tt-pl-textdomain'); ?></label>
                ...
                $the_query->the_post(); ?>
                    <div class="media">
                        <?php
                            ...
                            $image = wp_get_attachment_image_src($thumb_id, 'thumbnail'); // Get img URL.
                            //
                        ...
                        <div class="media-body">
                            ...
                            <h4><a href="<?php the_permalink(); ?>"><?php echo wp_trim_words(get_the_title(), $instance['title_length']); ?></a></h4>

                            <?php if ($instance[ 'show_meta' ] == 'yes') : ?>
                                <div class="entry-meta">
                                    <ul class="list-inline">
                                        <?php if ($instance[ 'post_type' ] == 'post'): ?>
                                            <li><?php printf('<a class="url fn n" href="%1$s">%2$s</a>', esc_url(get_author_posts_url(get_the_author_meta('ID'))), esc_html(get_the_author())); ?>
                                            </li>
